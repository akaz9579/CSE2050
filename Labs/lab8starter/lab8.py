class CustomSet:
    ...
    def __contains__(self, item):
        ...
        bucketIndex =  self._find_bucket(item)
        # return True if item is in bucket, false otherwise
        if item in self._L[bucketIndex]: return True 
        else: return False
    ...
    def remove(self, item):
        ...
        self._L[bucketIndex].remove(item)
        # update length
        self._len -=1
        ...
        if self._len <= 1/2*self._n_buckets and 1/2*self._n_buckets >= self._min_buckets:
            self._rehash(1/2*self._n_buckets)
        ...
        for bucket in self._L:
            ...
            for item in bucket:
                # Find the index of the new bucket for that item
                    new_bucket_index = hash(item) % int(new_buckets)
